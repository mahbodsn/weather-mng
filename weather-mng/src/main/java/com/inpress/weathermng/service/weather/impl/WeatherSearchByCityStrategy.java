...
public class WeatherSearchByCityStrategy implements WeatherSearchStrategy {
	...
	public WeatherSearchResult search(WeatherSearchModel model) throws BusinessException {
		...
		logger.debug("search start date -> [{}] and end date -> [{}], coordinate -> [{}]", startDate, endDate, model.cityName());
		var response = client.getWeatherByCityName(model.cityName(), startDate.format(DateTimeFormatter.ISO_DATE), endDate.format(DateTimeFormatter.ISO_DATE));
